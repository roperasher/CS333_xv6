...
testinvalidarray(void){
  ...
  printf(1, "beofre getprocs\n");
  ret = getprocs(1024, table);
  printf(1, "after getprocs call\n");
  free(table);
  if(ret >= 0){
...
testgetprocs(){
  ...
  if (ret == 0){
    ...
    printf(1, "before testinvalidarray\n");

    success  = testinvalidarray();
    printf(1, "testinvalidarray has run\n");
    success |= testprocarray( 1,  1);
printf(1, "testinvalidarray 1 has run\n");

    success |= testprocarray(16, 16);
printf(1, "testinvalidarray 16 has run\n");

    success |= testprocarray(64, 64);
printf(1, "testinvalidarray 64has run\n");

    success |= testprocarray(72, 64);
printf(1, "testinvalidarray 72 has run\n");
